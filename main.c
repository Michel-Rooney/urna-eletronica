...
void limparEleicao() {
    if (candidatos) free(candidatos);
    if (presidentes) free(presidentes);
    if (governadores) free(governadores);
    if (prefeitos) free(prefeitos);
...
void criarEleicao() {
    ...
    // Correção: Liberar a variável correta, não apenas 'candidatos'
    if (candidatos != NULL) free(candidatos);
    if (presidentes != NULL) free(presidentes); 
    if (governadores != NULL) free(governadores);
    if (prefeitos != NULL) free(prefeitos);
    ...
    if (candidatos == NULL || presidentes == NULL || governadores == NULL || prefeitos == NULL) {
