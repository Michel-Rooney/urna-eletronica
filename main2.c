#include <ctype.h>
#include <locale.h> // 1. Incluir a biblioteca
#include <stdbool.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
void menuEleicao() {
  ...
  int i, j, c, resultado, opcaoVoto, confirmacaoVoto;
  char *cargos[] = {"Presidente", "Governador", "Prefeito"};

  // Alocação para registro dos nomes dos eleitores
  ...
  for (i = 0; i < NElei; i++) {
    ...
    printf("=======================================\n");
    printf("               ELEIÇÃO                 \n");
    printf("       IDENTIFICAÇÃO DO ELEITOR        \n");
    ...
    // O eleitor deve votar para cada um dos 3 cargos
    for (c = 0; c < 3; c++) {
      int votoConfirmadoCargo = 0;
      while (!votoConfirmadoCargo) {
        limparTela();
        printf("ELEITOR: %s\n", eleitoresLog[i].nomeeleitor);
        printf("VOTAÇÃO PARA: %s\n", cargos[c]);
        printf("---------------------------------------\n");
        ...
        if (opcaoVoto == 1) {
          // Sub-menu para escolher candidato
          int encontrou = 0;
          limparTela();
          ...
          if (siglaDigitada == 0)
            continue; // Volta para o menu do cargo
          ...
          if (encontrou) {
            ...
            printf("\nCandidato não encontrado para este cargo!\n");
            sleep();
          }
